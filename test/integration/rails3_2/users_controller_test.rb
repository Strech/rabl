...
context "UsersController" do
  ...
  context "for index action" do
    ...
    end.equals { @users.map(&:created_at).map(&:utc).map(&:iso8601) }
  ...
  context "for show action" do
    ...
    # Attributes (custom name)
    asserts("contains registered_at") { json_output["person"]["registered_at"] }.equals { @user1.created_at.utc.iso8601 }
    # Node (renders based on attribute)
